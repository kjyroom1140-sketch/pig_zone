...
const SETTINGS_NAV = [
  { href: '/farm/admin', label: '농장 정보' },
  { href: '/farm/admin/staff', label: '직원 관리' },
  { href: '/farm/admin/schedule-masters', label: '일정 마스터' },
  { href: '/farm/admin/devices', label: '장치 관리' },
];
      ...
      >
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 2, paddingTop: 16 }}>
          {SETTINGS_NAV.map(({ href, label }) => {
            const active = pathname === href || (href !== '/farm/admin' && pathname?.startsWith(href));
            return (
              <button
                key={href}
                type="button"
